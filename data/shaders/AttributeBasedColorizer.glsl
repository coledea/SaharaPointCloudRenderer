...
vec4 colorization(vec4 color)
{
	...
	return_color.rgb = v_normal * 0.5 + 0.5;
#elif defined(USE_POSITION)
	return_color.rgb = (v_position - u_bbox_min) / (u_bbox_max - u_bbox_min);
#elif defined(USE_ID)
	const uint stepsize = 10000000;
	float greyscale = 0.5 - cos(float(double(v_id % stepsize) / double(stepsize)) * 2.0f * 3.141592653589793f) * 0.5f; // use periodic function -> makes it easier to spot point order/locality mismatches
	return_color.rgb = vec3(greyscale);
#elif defined(USE_SEGMENT)
	return_color.rgb = qualitativeColorForValue(v_segment);
#elif defined(USE_CUSTOM_FLOAT)
	return_color.rgb = vec3((v_custom - u_custom_value_min) / (u_custom_value_max - u_custom_value_min));
#elif defined(USE_CUSTOM_INT)
	if (u_use_linear_color_scale)
		return_color.rgb = vec3(float(v_custom - u_custom_value_min) / float(u_custom_value_max - u_custom_value_min));
	else
		return_color.rgb = qualitativeColorForValue(uint(v_custom));
#elif defined(USE_CUSTOM_UINT)
	if (u_use_linear_color_scale)
		return_color.rgb = vec3(float(v_custom - u_custom_value_min) / float(u_custom_value_max - u_custom_value_min));
	else
		return_color.rgb = qualitativeColorForValue(v_custom);
#elif defined(USE_CUSTOM_VEC)
	return_color.rgb = (v_custom - u_custom_value_min) / (u_custom_value_max - u_custom_value_min);
#endif

	return mix(return_color, color, u_original_color_factor);
}