...
const char* PRECOMPUTED_DISTANCE_FIELD_NAME = "scalar_significant_change"; // this is the attribute name assigned by CloudCompare for M3C2 differences. If such an attribute is present, we enable the Precomputed Differences mode.
	...
	, m_precomputed_change_highlighting_pass(opengl_context, camera, true)
{
	...
	auto comparison_modes = std::vector<QString>{ "Standard", "Side-to-Side", "Split View", "Real-Time Differences" };
	...
	{
		comparison_modes.push_back("Precomputed Differences");
	}
...
void MultiTemporalPointPrimitiveRasterizer::recompileShaders(const std::set<geometry::AttributeSpecification>& required_outputs)
{
	...
	if (m_comparison_mode_parameter->selectedEntry() == "Precomputed Differences")
	{
...
void MultiTemporalPointPrimitiveRasterizer::run(Framebuffer* framebuffer, int read_fbo_index)
{
	...
	else if (m_comparison_mode_parameter->selectedEntry() == "Real-Time Differences")
	{
		m_change_highlighting_pass.run();
	}

	else if (m_comparison_mode_parameter->selectedEntry() == "Precomputed Differences")
	{
