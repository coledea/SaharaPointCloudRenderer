	...
	, m_camera(camera)
{
	...
	updateColorScaleParameterVisibility();
...
void AttributeBasedColorizer::updateColorScaleParameterVisibility()
{
	const bool supports_qualitative_scale = selectedAttributeSupportsQualitativeColorScale();
	if (!supports_qualitative_scale)
	{
		m_color_scale_parameter->setValue(0u);
	}
	m_color_scale_parameter->setVisible(supports_qualitative_scale);
}

bool AttributeBasedColorizer::selectedAttributeSupportsQualitativeColorScale() const
{
	const auto attribute_selection = m_attribute_selection_parameter->selectedEntry();
	if (attribute_selection == "depth")
	{
	...
	const auto attribute_type = m_pointcloud_provider->attributeMetadata(attribute_selection)->type;
	return attribute_type == geometry::AttributeType::Int || attribute_type == geometry::AttributeType::Uint;
}
